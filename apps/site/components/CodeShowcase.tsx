        ...
        <div className="mt-12 lg:mt-16 grid grid-cols-12 gap-8">
          <div className="col-span-12 lg:col-span-3 order-2 lg:order-1 text-[12px] leading-[1.7] text-fg-muted">
            <p className="border-l-2 border-phosphor pl-4">
              The SDK is one entry point. Every layer hangs off it:{' '}
              <span className="text-fg">identity, wallet, discovery, marketplace, payment, reputation</span>.
            </p>
          ...
          <div className="col-span-12 lg:col-span-9 order-1 lg:order-2 relative">
            ...
            <pre className="code">
              ...
              <Hl type="typ">AgentMesh</Hl>.<Hl type="fn">create</Hl>({'{'} chain<Hl type="pun">:</Hl>{' '}
              <Hl type="str">'anvil'</Hl>, ownerKey {'}'});
              {'\n\n'}
              <Hl type="com">{'// 2. smart account + spending policy + identity'}</Hl>
              {'\n'}
              <Hl type="key">await</Hl> mesh.wallet.<Hl type="fn">create</Hl>();
              {'\n'}
              <Hl type="key">await</Hl> mesh.wallet.<Hl type="fn">fund</Hl>(<Hl type="fn">parseEther</Hl>(
              <Hl type="str">'1'</Hl>));
              {'\n'}
              <Hl type="key">await</Hl> mesh.wallet.<Hl type="fn">installPolicy</Hl>({'{'}
              {'\n'} dailyLimitWei<Hl type="pun">:</Hl> <Hl type="fn">parseEther</Hl>(<Hl type="str">'1'</Hl>
              ),
              {'\n'} perTxLimitWei<Hl type="pun">:</Hl> <Hl type="fn">parseEther</Hl>(
              <Hl type="str">'0.1'</Hl>),
              {'\n'}
              {'}'});
              {'\n'}
              ...
              <Hl type="pun">:</Hl> [<Hl type="str">'consumer'</Hl>],
              {'\n'} metadataURI<Hl type="pun">:</Hl> <Hl type="str">'ipfs://…'</Hl>,{'\n'}
              {'}'});
              {'\n\n'}
              <Hl type="com">{'// 3. discover, pay, fetch — all behind one fetch()'}</Hl>
              {'\n'}
              <Hl type="key">const</Hl> [provider] <Hl type="pun">=</Hl> <Hl type="key">await</Hl>{' '}
              mesh.discovery.
